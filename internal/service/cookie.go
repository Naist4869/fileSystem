...
func NewSimpleJar() (jar *SimpleJar, cf func(), err error) {
	var (
		ct  paladin.TOML
		cfg JarOption
	)
	if err = paladin.Get("cookie.toml").Unmarshal(&ct); err != nil {
		return
	}
	if err = ct.Get("JarOption").UnmarshalTOML(&cfg); err != nil {
		return
	}
	if cfg.Filename == "" {
		cfg.JarType = JarMemory
	}
